...
class RobertaForMultiLabelSequenceClassification(RobertaPreTrainedModel):
    def __init__(self, config):
        ...
        self.output_layer = torch.nn.Linear(config.hidden_size, config.num_labels)
...
if __name__ == "__main__":
    ...
    if args.test_only:
        ...
        model = RobertaForMultiLabelSequenceClassification.from_pretrained(model_path, num_labels=len(label_map))
    ...
    training_args = TrainingArguments(
        ...
        eval_steps=100,                    # Evaluate the model every X steps

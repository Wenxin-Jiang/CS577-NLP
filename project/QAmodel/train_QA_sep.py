...
def indices_to_text(pred_indices, input_ids, tokenizer):
    pred_texts = []
    for indices, ids in zip(pred_indices, input_ids):
        start_idx, end_idx = indices
        tokens = ids[start_idx:end_idx+1]
...
if __name__ == "__main__":
    ...
    with open("/depot/davisjam/data/wenxin/CS577-NLP/project/QAmodel/RemovedQ2Q3_dataset.json", "r") as f:
    ...
    for epoch in range(num_epochs):
        ...
        train_preds, train_true_labels, train_input_ids, train_exact_match, train_f1 = evaluate(model, train_loader)
        val_preds, val_true_labels, val_input_ids, val_exact_match, val_f1 = evaluate(model, eval_loader)
        test_preds, test_true_labels, test_input_ids, test_exact_match, test_f1 = evaluate(model, test_loader)
    ...
    train_pred_texts = indices_to_text(train_preds, train_input_ids, tokenizer)
    val_pred_texts = indices_to_text(val_preds, val_input_ids, tokenizer)
    test_pred_texts = indices_to_text(test_preds, test_input_ids, tokenizer)

    # Save train, val, and test predictions to CSV files
    df_train = pd.DataFrame({'true_labels': train_true_labels, 'pred_labels': train_preds, 'pred_texts': train_pred_texts})
    df_train.to_csv("train_predictions.csv", index=False)

    df_val = pd.DataFrame({'true_labels': val_true_labels, 'pred_labels': val_preds, 'pred_texts': val_pred_texts})
    df_val.to_csv("val_predictions.csv", index=False)

    df_test = pd.DataFrame({'true_labels': test_true_labels, 'pred_labels': test_preds, 'pred_texts': test_pred_texts})
    ...
    iterations_train_match = range(1, len(train_exact_matches) + 1)
    iterations_val_match = range(1, len(val_exact_matches) + 1)
    iterations_test_match = range(1, len(test_exact_matches) + 1)

    # Train, validation, and test loss plots
plt.plot(iterations_train, train_losses, label='Training loss')
plt.title(f'Training Loss (lr={lr}, num_epochs={num_epochs})')
plt.xlabel('Iterations')
plt.ylabel('Loss')
plt.savefig(f'../RemovedQ2Q3_dataset/train_loss_plot_QA_{num_epochs}_{lr}.png')
plt.show()
plt.close()

plt.plot(iterations_val, val_losses, label='Validation loss')
plt.title(f'Validation Loss (lr={lr}, num_epochs={num_epochs})')
plt.xlabel('Iterations')
plt.ylabel('Loss')
plt.savefig(f'../RemovedQ2Q3_dataset/val_loss_plot_QA_{num_epochs}_{lr}.png')
plt.show()
plt.close()

plt.plot(iterations_test, test_losses, label='Test loss')
plt.title(f'Test Loss (lr={lr}, num_epochs={num_epochs})')
plt.xlabel('Iterations')
plt.ylabel('Loss')
plt.savefig(f'../RemovedQ2Q3_dataset/test_loss_plot_QA_{num_epochs}_{lr}.png')
plt.show()
plt.close()

# Train, validation, and test exact match plots
plt.plot(range(1, len(train_exact_matches) + 1), train_exact_matches, label='Train exact match')
plt.title(f'Train Exact Match (lr={lr}, num_epochs={num_epochs})')
plt.xlabel('Iterations')
plt.ylabel('Exact Match')
plt.savefig(f'../RemovedQ2Q3_dataset/train_exact_match_plot_QA_{num_epochs}_{lr}.png')
plt.show()
plt.close()

plt.plot(range(1, len(val_exact_matches) + 1), val_exact_matches, label='Validation exact match')
plt.title(f'Validation Exact Match (lr={lr}, num_epochs={num_epochs})')
plt.xlabel('Iterations')
plt.ylabel('Exact Match')
plt.savefig(f'../RemovedQ2Q3_dataset/val_exact_match_plot_QA_{num_epochs}_{lr}.png')
plt.show()
plt.close()

plt.plot(range(1, len(test_exact_matches) + 1), test_exact_matches, label='Test exact match')
plt.title(f'Test Exact Match (lr={lr}, num_epochs={num_epochs})')
plt.xlabel('Iterations')
plt.ylabel('Exact Match')
plt.savefig(f'../RemovedQ2Q3_dataset/test_exact_match_plot_QA_{num_epochs}_{lr}.png')
plt.show()
plt.close()

# logger.debug(train_f1_scores)
# logger.debug(val_f1_scores)
# logger.debug(test_f1_scores)
# logger.debug(train_exact_matches)
# logger.debug(val_exact_matches)
# logger.debug(test_exact_matches)
# Train, validation, and test F1 score plots
plt.plot(range(1, len(train_f1_scores) + 1), train_f1_scores, label='Train F1 score')
plt.title(f'Train F1 Score (lr={lr}, num_epochs={num_epochs})')
plt.xlabel('Iterations')
plt.ylabel('F1 Score')
plt.savefig(f'../RemovedQ2Q3_dataset/train_f1_score_plot_QA_{num_epochs}_{lr}.png')
plt.show()
plt.close()

plt.plot(range(1, len(val_f1_scores) + 1), val_f1_scores, label='Validation F1 score')
plt.title(f'Validation F1 Score (lr={lr}, num_epochs={num_epochs})')
plt.xlabel('Iterations')
plt.ylabel('F1 Score')
plt.savefig(f'../RemovedQ2Q3_dataset/val_f1_score_plot_QA_{num_epochs}_{lr}.png')
plt.show()
plt.close()

plt.plot(range(1, len(test_f1_scores) + 1), test_f1_scores, label='Test F1 score')
plt.title(f'Test F1 Score (lr={lr}, num_epochs={num_epochs})')
plt.xlabel('Iterations')
plt.ylabel('F1 Score')
plt.savefig(f'../RemovedQ2Q3_dataset/test_f1_score_plot_QA_{num_epochs}_{lr}.png')
plt.show()
plt.close()
